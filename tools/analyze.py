...
def analyze_corpus(generator: Generator, verbose: bool = False) -> Dict:
    ...
    analysis = {
        ...
        "unique_syllables": len(set(str(s) for name in generator.names for s in name.syllables)),
        "avg_syllables_per_name": sum(len(name.syllables) for name in generator.names) / len(generator.names),
...
def main():
    ...
    parser.add_argument('--top-syllables', type=int, default=20,
    ...
    try:
        ...
        if args.json:
            ...
            print(f"Average syllables per name: {analysis['avg_syllables_per_name']:.2f}")
            
            if args.verbose and 'name_length_stats' in analysis:
                stats = analysis['name_length_stats']
                print(f"Name length: min={stats['min']}, max={stats['max']}, avg={stats['avg']:.2f}")
                
                stats = analysis['syllable_count_stats']
                print(f"Syllable count: min={stats['min']}, max={stats['max']}, avg={stats['avg']:.2f}")
            
            print(f"\nTop {args.top_syllables} syllables by frequency:")
            top_syllables = sorted(analysis['syllable_frequency'].items(), 
                                 key=lambda x: x[1], reverse=True)[:args.top_syllables]
            
            for syllable, count in top_syllables:
                percentage = (count / sum(analysis['syllable_frequency'].values())) * 100
