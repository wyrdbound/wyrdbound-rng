...
class TestJapaneseNameSegmenter:
    ...
    def test_segment_double_syllable_names(self):
        ...
        for name in double_syllable_names:
            ...
            assert syllables is not None
            assert isinstance(syllables, list)
            assert len(syllables) == 2, f"{name} should be 2 syllables, got {len(syllables)}: {[str(s) for s in syllables]}"
        
        for name in single_syllable_names:
            syllables = JapaneseNameSegmenter.segment(name)
            assert syllables is not None
            assert isinstance(syllables, list)
            assert len(syllables) == 1, f"{name} should be 1 syllable, got {len(syllables)}: {[str(s) for s in syllables]}"
    ...
    def test_segment_quadruple_syllable_names(self):
        ...
        three_syllable_names = ['Bungoro', 'Hattori', 'Echizen']  # Gemination and improved segmentation
        
        for name in quadruple_syllable_names:
            syllables = JapaneseNameSegmenter.segment(name)
            assert syllables is not None
            assert isinstance(syllables, list)
            assert len(syllables) == 4, f"{name} should be 4 syllables, got {len(syllables)}: {[str(s) for s in syllables]}"
        
        for name in three_syllable_names:
            syllables = JapaneseNameSegmenter.segment(name)
            assert syllables is not None
            assert isinstance(syllables, list)
            assert len(syllables) == 3, f"{name} should be 3 syllables, got {len(syllables)}: {[str(s) for s in syllables]}"

    def test_segment_long_names(self):
        """Test segmenting very long names."""
        # Updated to reflect improved Japanese segmentation
        long_names = ['Chousokabe', 'Shikanosuke', 'Kirigakure']  # These are 5 syllables
        ...
        for name in long_names:
            ...
            assert syllables is not None
            assert isinstance(syllables, list)
            assert len(syllables) >= 5, f"{name} should be >=5 syllables, got {len(syllables)}: {[str(s) for s in syllables]}"
    ...
    def test_segment_doubled_n_as_in_geni(self):
        ...
        assert syllables is not None
        assert isinstance(syllables, list)
        assert len(syllables) == 2, f"Gen'i should be 2 syllables, got {len(syllables)}: {[str(s) for s in syllables]}"
