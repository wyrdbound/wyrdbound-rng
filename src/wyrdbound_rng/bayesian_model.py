...
class BayesianModel:
    ...
    def train(self, names: List, names_file: str, segmenter_type: str) -> None:
        ...
        if self._load_from_cache(cache_key):
            print(f"Loaded Bayesian probabilities from cache ({len(self.syllables)} syllables)")
            return
        ...
        for syllable in self.syllables:
            ...
            self.start_probs[syllable] = (count + self.smoothing_alpha) / (total_starts + vocab_size * self.smoothing_alpha)
        
        # End probabilities  
        total_ends = sum(end_counts.values())
        for syllable in self.syllables:
            count = end_counts.get(syllable, 0)
            self.end_probs[syllable] = (count + self.smoothing_alpha) / (total_ends + vocab_size * self.smoothing_alpha)
        ...
        for current_syl in self.syllables:
            ...
            for next_syl in self.syllables:
                ...
                # Laplace smoothing
                probability = (count + self.smoothing_alpha) / (total_transitions + vocab_size * self.smoothing_alpha)
