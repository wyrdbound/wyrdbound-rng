...
try:
    ...
    raise ImportError("PyYAML is required for name file loading. Install with: pip install PyYAML")
...
class NameFileLoader:
    ...
    def _get_segmenter(self, segmenter_name):
        ...
        segmenter_class = segmenter_map.get(segmenter_name.lower(), FantasyNameSegmenter)
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as yaml_file:
                ...
                if not isinstance(data, dict):
                    raise FileLoadError(f"Invalid YAML format in '{file_path}': expected dictionary")
                ...
                if not isinstance(names_list, list):
                    raise FileLoadError(f"Invalid YAML format in '{file_path}': 'names' must be a list")
