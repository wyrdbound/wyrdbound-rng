...
def main():
    ...
    parser.add_argument('-l', '--length', type=int, default=12,
                       help='Maximum name length (default: 12)')
    parser.add_argument('-a', '--algorithm', choices=['very_simple', 'simple', 'bayesian'],
                       default='simple', help='Generation algorithm (default: simple)')
    parser.add_argument('-s', '--segmenter', choices=['fantasy', 'japanese'],
                       default='fantasy', help='Segmentation method (default: fantasy)')
    ...
    parser.add_argument('--min-probability', type=float, default=1.0e-8,
    ...
    parser.add_argument('--probabilities', type=str, metavar='SYLLABLE',
    ...
    try:
        ...
        if args.probabilities:
            if args.algorithm != 'bayesian':
                print("Warning: Probability analysis works best with bayesian algorithm")
            ...
            for syllable in syllables:
                ...
                if not prob_info:
                    print(f"No probability information available for syllable '{syllable}'")
                    ...
                    print(f"Start probability: {prob_info.get('start_probability', 0.0):.4f}")
                    print(f"End probability: {prob_info.get('end_probability', 0.0):.4f}")
        ...
        for i in range(args.number):
            ...
            if args.show_sources and hasattr(name, 'source_names') and name.source_names:
